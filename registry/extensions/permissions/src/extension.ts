type Mode = "suggest" | "auto-edit" | "full-auto";
interface FrontendCapabilities { interactive: boolean; interaction_kinds: string[] }
...
interface InteractionResponse { id: string; owner: string; generation: number; action_id: string }
interface Pending { invocationId: string; scope: string }

let mode: Mode = "suggest";
let pending = new Map<string, Pending>();
let approvedScopes = new Set<string>();

function isMode(value: unknown): value is Mode {
  return value === "suggest" || value === "auto-edit" || value === "full-auto";
}

function asks(effect: ToolEffect): boolean {
  if (mode === "full-auto") return false;
  if (mode === "auto-edit") return effect === "execute" || effect === "unknown";
  return effect !== "read";
}

function applySettings(value: Json): void {
...
function objectArguments(invocation: ToolInvocation): Record<string, Json> {
  return invocation.arguments && typeof invocation.arguments === "object" && !Array.isArray(invocation.arguments)
    ? invocation.arguments as Record<string, Json>
    : {};
}

function scopeOperation(invocation: ToolInvocation): Json {
...
function scope(invocation: ToolInvocation): string {
...
function summary(invocation: ToolInvocation): string {
...
function approvalSupported(frontend: FrontendCapabilities): boolean {
...
tokio.defineExtension({
  loadState({ userState, sessionState, settings, startupSettings }) {
    ...
    const scopes = tokio.storage.decodeJson<Json>(sessionState, []);
    if (Array.isArray(scopes)) {
      approvedScopes = new Set(scopes.filter((value): value is string => typeof value === "string"));
    }
  },

  restoreSessionState(bytes) {
    const scopes = tokio.storage.decodeJson<Json>(bytes, []);
  ...
  onCommand(handler) {
    if (handler !== "permissions_command") return [];
  ...
  authorizeTool(handler, value) {
    ...
    const invocation = value as unknown as ToolInvocation;
  ...
  onInteractionResponse(handler, invocationId, value) {
    ...
    const response = value as unknown as InteractionResponse;
    ...
    if (!request) return { decision: "deny", reason: "stale or duplicate interaction response", actions: [] };
    if (request.invocationId !== invocationId) {
      return { decision: "deny", reason: "interaction does not own this invocation", actions: [] };
    }
    if (response.action_id === "allow_once") return { decision: "allow", actions: [] };
