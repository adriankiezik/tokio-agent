...
impl Guest for Goal {
    fn authorize_tool(_handler: String, _invocation_json: String) -> String {
        r#"{"decision":"deny","reason":"goal is not a tool gate","actions":[]}"#.into()
    }
    fn on_interaction_response(
        ...
        r#"{"decision":"deny","reason":"goal has no interactions","actions":[]}"#.into()
    }
    fn load_state(
        _user_state: Vec<u8>,
    ...
    fn on_command(_handler: String, arguments: String) -> String {
        ...
        match arguments.trim() {
            ...
            "resume" if state.active => {
                ...
                actions(vec![
                    ExtensionAction::SubmitPrompt {
                        text: continuation(&state.objective),
                        automatic: true,
                    },
            ...
            control @ ("pause" | "resume") => {
                notice(&format!("Cannot {control}; no goal is active"))
            }
            objective => {
    ...
    fn on_event(event_json: String) -> String {
        ...
        match event {
            SessionEvent::TurnFinished { .. } if !state.paused => {
                actions(vec![ExtensionAction::SubmitPrompt {
                    text: continuation(&state.objective),
                    automatic: true,
                }])
            }
...
fn persist(state: &State) -> Vec<u8> {
    serde_json::to_vec(&(state.objective.as_str(), state.active, state.paused)).unwrap_or_default()
}
...
fn tool() -> ToolDescriptor {
    ToolDescriptor {
        ...
        effect: tokio_agent_extension_api::ToolEffect::Read,
    }
}
...
fn continuation(objective: &str) -> String {
    format!(
        "Continue working toward the active goal. Verify progress and keep going until complete or genuinely blocked.\n\nGoal: {objective}"
    )
}
...
fn notice(text: &str) -> String {
    actions(vec![ExtensionAction::ShowNotice {
