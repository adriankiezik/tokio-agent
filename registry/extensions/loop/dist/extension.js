...
function parseInterval(value) {
    ...
    return Number.isSafeInteger(milliseconds) && milliseconds >= 10000 ? milliseconds : undefined;
}
function formatDuration(milliseconds) {
    if (milliseconds < MINUTE_MS)
        return `${Math.ceil(milliseconds / SECOND_MS)}s`;
    if (milliseconds < HOUR_MS)
        return `${Math.ceil(milliseconds / MINUTE_MS)}m`;
    if (milliseconds < DAY_MS)
        return `${Math.ceil(milliseconds / HOUR_MS)}h`;
    return `${Math.ceil(milliseconds / DAY_MS)}d`;
}
function restore(bytes) {
    const saved = tokio.storage.decodeJson(bytes, null);
